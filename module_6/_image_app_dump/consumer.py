...
def _load_etl_modules(etl_dir: Path) -> list[Any]:
    ...
    for py in sorted(etl_dir.rglob("*.py")):
        if py.name.startswith("_"):
            continue
        # avoid importing compiled cache paths, just in case
...
def _collect_q_functions(modules: list[Any]) -> dict[str, Callable[[psycopg.Connection], Any]]:
    """
    Find functions named q1_*, q2_*, ... q9_* across ETL modules.
    Returns mapping like {"q1": func, "q2": func, ...}.
    If multiple candidates exist for same q#, the first one found wins.
    """
...
def _normalize_results(raw: dict[str, Any]) -> dict[str, list[Any]]:
    """
    Your templates expect results.q1..results.q9 and often index [0].
...
def handle_recompute_analytics(conn: psycopg.Connection, payload: dict) -> None:
    ...
    with conn.cursor() as cur:
        cur.execute(upsert_sql, ("latest", json.dumps(results)))
...
def handle_scrape_new_data(conn: psycopg.Connection, payload: dict) -> None:
    """
    Best-effort: call an ETL scrape/ingest function if your etl/ folder has one.
    If none exists, write a watermark so you still have evidence the task ran.
    """
    etl_dir = Path("/app/etl")
    modules = _load_etl_modules(etl_dir)
    ...
    for m in modules:
        for name in candidates:
            fn = getattr(m, name, None)
            if callable(fn):
                _log(f"running ETL function: {m.__name__}.{name}()")
                fn(conn)  # expects (conn) signature
                called = True
                break
        if called:
            break

    # Always update watermark as evidence of execution
    source = payload.get("source") or "web"
    last_seen = payload.get("last_seen") or time.strftime("%Y-%m-%d %H:%M:%S")
    ...
    if not called:
        _log("no ETL scrape function found; watermark updated only")
...
def main() -> None:
    ...
    while True:
        try:
            ...
            def on_message(channel, method, properties, body) -> None:  # type: ignore[no-untyped-def]
